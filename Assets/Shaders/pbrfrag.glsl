...
void main (void) {
	...
	fragColour [0] = mix(texture2D(diffuseTex, vec2(IN.texCoord.x, 1 - IN.texCoord.y) ), vec4((reflection.xyz * metalness.xyz), 1.0), metalness.r) * objectColour;
	fragColour [1] = vec4(normal.xyz * 0.5 + 0.5, 1.0);
	fragColour [2] = texture2D(specularTex , vec2(IN.texCoord.x, 1 - IN.texCoord.y));
}