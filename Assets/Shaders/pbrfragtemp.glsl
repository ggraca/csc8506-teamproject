...
void main (void) {
	...
	vec3 normal = normalize(TBN * normalize(Vec3(0.0, 0.0, 1.0)) * 2.0 - 1.0));
	vec4 metalness = vec4(1.0, 0.0, 0.0, 1.0);

	vec3 incident = normalize(IN.worldPos - cameraPos);
	vec4 reflection = vec4(1.0, 1.0, 1.0, 1.0);
	
	fragColour [0] = mix(texture2D(mainTex , IN.texCoord), vec4((reflection.xyz * metalness.xyz), 1.0), metalness.r) * objectColour;
