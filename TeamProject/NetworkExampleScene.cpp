...
void NetworkExampleScene::ResetWorld() {
	...
	for (int i = -size; i <= size; i++) {
		for (int j = -size; j <= size; j++) {
			world->LateInstantiate(new ConePrefab(Vector3(50, 100, 50), Vector3(i * 100, 130, j * 30), Quaternion::AxisAngleToQuaternion(Vector3(0, 0, 0), 0), 10, 0.5, 0.5));
		}
	}

	for (int i = -size; i <= size; i++) {
		for (int j = -size; j <= size; j++) {	
			world->LateInstantiate(new CylinderPrefab(Vector3(50, 100, 50), Vector3(i * 100, 190, j * 30), Quaternion::AxisAngleToQuaternion(Vector3(0, 0, 0), 0), 10, 0.5, 0.5));
		}
	}
